    ...
    <ul class="list-none m-0 p-0 flex items-center gap-2">
        ...
        <li class="relative group">
            <a href="{{ $item->getUrl() ?? '#' }}" @if (count($children)> 0)
                ...
                @endif
                class="{{ $item->isActive() ? 'inline-flex items-center gap-1 px-4 py-2 rounded-md bg-white text-orange-600 font-semibold' : 'inline-flex items-center gap-1 px-4 py-2 rounded-md text-orange-100 hover:bg-orange-500 hover:text-white transition-colors' }}">
                {{ $item->getLabel() }}
            </a>

            @if (count($children) > 0)
            <ul class="list-none m-0 absolute left-0 top-full z-50 hidden w-64 space-y-2 rounded-md border border-orange-400 bg-orange-600 p-3 shadow-lg group-hover:block group-focus-within:block">
